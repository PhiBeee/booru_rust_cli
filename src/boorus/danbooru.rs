...
const REQUEST_CAP: i64 = 200;

pub fn run_danbooru(config: BooruConfig) {
    ...
    for page in 1..=config.pid {
        ...
        let get_request = format!("https://danbooru.donmai.us/posts.json?limit={}&tags={}&page={}", images_left, tags, page);
        // Get image urls
    ...
    println!("\r\nFinished! You can find the images in images/danbooru");
}

#[tokio::main]
async fn get_images(get_request: String) -> Vec<DanbooruPost> {
...
fn download_images(posts: Vec<DanbooruPost>, mut images_left: i64) -> i64{
    for post in posts {
        let image = post;
        ...
        match image.file_url {
            Some(url) => {
                ...
                let file_name = format!("images/danbooru/{image_id}.{file_extension}");
